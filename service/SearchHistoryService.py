...
class SearchHistoryService:
    ...
    def is_exist_query(db, query):
        db.cursor.execute(
            "SELECT id, count FROM search_history WHERE query LIKE %s", (query,)
        )
    ...
    def update_existed_query(db, field_id, count):
        db.cursor.execute(
            "UPDATE search_history SET count = %s WHERE id = %s", (count, field_id)
        )
    ...
    def write_new_history_query(db, query, count=1):
        db.cursor.execute(
            "INSERT INTO search_history (query, count) VALUES (%s, %s);", (query, count)
        )
        db.connection.commit()

    @staticmethod
    def get_search_history(db):
        db.cursor.execute("SELECT query FROM search_history ORDER BY count DESC LIMIT 10;")

        return [SearchHistory(query[0]) for query in db.cursor.fetchall()]
